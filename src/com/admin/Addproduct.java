...
public class Addproduct extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int dateflag=0,dateflag_mfg=0;
		int Mflag=0,Eflag=0;
		dateflag_mfg=DateLookup.Mfg_date(mfgdate);
		System.out.println("************");
		dateflag=DateLookup.exp_date(Expirydate);
		if(dateflag_mfg==0)
		{
			Mflag=1;
		}
		...
		{
			pw.println("<html><script>alert('MFG Date Invalid');</script><body>");
			pw.println("");
			pw.println("</body></html>");
			response.sendRedirect("UpdateProduct.jsp?MFGInvalid");
		}
		if(dateflag==0)
		{
			Eflag=1;
		...
		{
			pw.println("<html><script>alert('Date Invalid');</script><body>");
			pw.println("");
			pw.println("</body></html>");
			response.sendRedirect("UpdateProduct.jsp?ExpiryInvalid");
		}
		if(Mflag==1 && Eflag==1){
		try {
